...
void pic_init(void) {
  ...
  pic_set_all_irqs(1); // Disable all irqs
...
void pic_enable_irq(unsigned char irq) {
  ...
  if (irq < 8) {
    // Master
    unsigned char mask = inb(PIC1_DATA_PORT);
    mask &= ~(1 << irq);
    outb(PIC1_DATA_PORT, mask);
  } else {
    // Slave
    unsigned char mask = inb(PIC2_DATA_PORT);
    mask &= ~(1 << (irq - 8));
...
/**
 * @param val 1 = disable all irqs (master and slave), 0 = enable all irqs
 * (master and slave)
 */
